...
#'   provided the configuration value.
#' @return The configuration value if found; otherwise `default`.
...
get_config <- function(key,
  ...
  safe_read <- function(path) {
    ...
    # Read whole config file as a list; don't request a key yet.
  ...
  candidates <- candidates[!duplicated(path.expand(candidates))]

  for (file in candidates) {
    cfg <- safe_read(file)
    if (is.null(cfg)) {
      if (isTRUE(verbose)) message(sprintf("[config] skip: %s", normalizePath(file, mustWork = FALSE)))
      next
    }

    val <- get_from_list(cfg, key)
    if (!is.null(val)) {
      if (isTRUE(verbose)) message(sprintf("[config] '%s' from %s", key, normalizePath(file, mustWork = FALSE)))
      return(val)
    }

    if (isTRUE(verbose)) message(sprintf("[config] key '%s' not in %s", key, normalizePath(file, mustWork = FALSE)))
  }

  if (isTRUE(verbose)) {
    message(sprintf("[config] key '%s' not found. candidates: %s",
                    key, paste(normalizePath(candidates, mustWork = FALSE), collapse = ", ")))
  }
